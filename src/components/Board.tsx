...
export default function Board() {
  ...
  const userBoard = useGameStore((s) => s.userBoard);
  const steps = useGameStore((s) => s.steps);
  const currentStepIndex = useGameStore((s) => s.currentStepIndex);
  const selectedCell = useGameStore((s) => s.selectedCell);
  const selectCellAction = useGameStore((s) => s.selectCell);

  const board = mode === 'play' ? userBoard : currentBoard;

  const highlights =
    mode === 'coach' && currentStepIndex >= 0 && currentStepIndex < steps.length
      ? steps[currentStepIndex].move.evidence.highlights
      : [];
  const highlightMap = buildHighlightMap(highlights);

  const conflictSet = mode === 'play' ? findConflicts(userBoard) : new Set<CellIndex>();
        ...
        {board.cells.map((cell) => {
          ...
          return (
            ...
            >
              <Cell
                cell={cell}
                highlight={mode === 'coach' ? highlightMap.get(cell.index) : undefined}
                selected={mode === 'play' && selectedCell === cell.index}
                conflict={mode === 'play' && conflictSet.has(cell.index)}
                interactive={mode === 'play'}
                onClick={() => selectCellAction(cell.index)}
              />
            </div>
          );
        })}
